...
static void process_tx_irq(const struct device *dev)
{
	...
	if (tx_head_sent >= 0 && ack == tx_head.header.msg_id) {
		ack = -1;
		tx_head_sent = -1;
	}

	/* always send ack first */
	if (tx_head_sent <= 0 && pending_ack >= 0) {
	...
	if (tx_head_sent < 0) {
		if (k_msgq_get(&tx_queue, &tx_head, K_NO_WAIT) == 0) {
			tx_head_sent = 0;
	...
	if (tx_head_sent >= 0) {
		...
		if (tx_head_sent < msg_size) {
			filled = uart_fifo_fill(bus, (uint8_t*)&tx_head + tx_head_sent,
...
{
	static struct RoachMessage recv_buf;
	static int tail = 0;
	const struct one_wire_uart_config *config = dev->config;
	const struct device *bus = config->bus;

	int bytes_read = uart_fifo_read(bus, (uint8_t*)&recv_buf + tail,
					sizeof(recv_buf) - tail);
	...
	while (true) {
		...
		int msg_size = HEADER_SIZE + recv_buf.header.payload_len;

		/* size too large, looks like an invalid message */
		if (msg_size > sizeof(recv_buf)) {
			/* erase current marker and find next one */
			recv_buf.header.magic = 0;
			continue;
		}
		...
		if (checksum(&recv_buf) != 0) {
			/* erase current marker and find next one */
			recv_buf.header.magic = 0;
			continue;
		}

		if (recv_buf.header.sender != config->id) {
			uint8_t msg_id = recv_buf.header.msg_id;

			if (recv_buf.header.ack) {
				ack = msg_id;
			} else {
				k_msgq_put(&rx_queue, &recv_buf, K_NO_WAIT);
		...
		memmove(&recv_buf, (uint8_t*)&recv_buf + msg_size, tail - msg_size);
		tail -= msg_size;
	}
}
